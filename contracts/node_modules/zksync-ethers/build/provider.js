...
class Provider extends ethers_1.ethers.providers.JsonRpcProvider {
    ...
    async getWithdrawTx(transaction) {
        ...
        const ntv = new ethers_1.Contract(utils_1.L2_NATIVE_TOKEN_VAULT_ADDRESS, utils_1.L2_NATIVE_TOKEN_VAULT_ABI, this);
        ...
        const isTokenL1Native = originChainId.eq(l1ChainId) || tx.token === utils_1.ETH_ADDRESS_IN_CONTRACTS;
        let populatedTx;
        ...
        if (tx.bridgeAddress && isTokenL1Native) {
            ...
            populatedTx = await bridge.populateTransaction.withdraw(tx.to, tx.token, tx.amount, tx.overrides);
        }
        else {
            const bridge = this.connectL2AssetRouter();
            const assetData = (0, utils_1.encodeNativeTokenVaultTransferData)(tx.amount, tx.to, tx.token);
            populatedTx = await bridge.populateTransaction.withdraw(assetId, assetData, tx.overrides);
        }
        if (tx.paymasterParams) {
            return {
                ...populatedTx,
